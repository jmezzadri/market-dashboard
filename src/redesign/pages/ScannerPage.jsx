...
import { MT_SCANNER } from "../data/mock";

export default function ScannerPage({ setPage, openTicker }) {
      ...
      <section className="mt-pagehero">
        ...
        <div className="sc-results">
          ...
          <div className="sc-buckets">
            <button
              className={`sc-bucket sc-bucket--score7 ${bucket === "7+" ? "on" : ""}`}
            ...
            <button
              className={`sc-bucket sc-bucket--score5 ${bucket === "5-6" ? "on" : ""}`}
            ...
            <button
              className={`sc-bucket sc-bucket--score3 ${bucket === "3-4" ? "on" : ""}`}
      ...
      <section className="mt-pagesection" style={{ paddingTop: 8 }}>
        <div className="sc-toolbar">
          <div className="mt-pillgroup">
            <button className={`mt-pill ${bucket === "all" ? "on" : ""}`} onClick={() => setBucket("all")}>
              All
            </button>
            <button className={`mt-pill ${bucket === "7+" ? "on" : ""}`} onClick={() => setBucket("7+")}>
              Score 7+
            </button>
            <button className={`mt-pill ${bucket === "5-6" ? "on" : ""}`} onClick={() => setBucket("5-6")}>
              Score 5–6
            </button>
            <button className={`mt-pill ${bucket === "3-4" ? "on" : ""}`} onClick={() => setBucket("3-4")}>
              Score 3–4
            </button>
          </div>
          <span className="sc-shortnote">
            <Tip content="Engine doesn't yet output short signals — long-only universe today.">Long signals only</Tip>
          </span>
          <span style={{ flex: 1 }} />
          ...
          <div className="sc-colpicker mt-fade">
            ...
            <div className="sc-colgrid">
              {[
                ...
                <label
                  key={name}
                  className={`sc-coltoggle ${on ? "on" : ""} ${lock ? "locked" : ""}`}
                >
      ...
      <section className="mt-pagesection" style={{ paddingTop: 16 }}>
        <div className="mt-card">
          <div className="mt-sectionhead" style={{ marginBottom: 12 }}>
            ...
            <button className="mt-btn mt-btn--ghost" onClick={() => setPage("methodology")}>
              Full methodology →
            </button>
          </div>
