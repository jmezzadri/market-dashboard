...
import { FreshnessChip, Tip } from "../atoms";
import ScoreDial from "../components/ScoreDial";
import BigHistoryChart from "../components/BigHistoryChart";
import { MT_SCANNER, MT_POSITIONS, gen } from "../data/mock";
import { breakdownForTicker } from "../data/score";

const TF_MAP = { "1M": 21, "3M": 63, "6M": 126, "1Y": 252, "5Y": 1260, Max: 5000 };
...
export default function TickerPage({ symbol, onBack, openTicker }) {
  ...
  const row =
    MT_SCANNER.find((r) => r.ticker === symbol) ||
    MT_POSITIONS.find((p) => p.ticker === symbol);
  const mock = useMemo(
    () =>
      row || {
        ticker: symbol,
  ...
  const series = useMemo(
    ...
    [tf, mock.price, mock.chg, mock.ticker]
  );
    ...
    <div className="tk-page">
      <section className="mt-pagehero tk-hero">
        <div>
          <div className="tk-back">
            <button className="mt-btn mt-btn--ghost" onClick={onBack}>← Back to scanner</button>
          ...
          <div className="tk-priceblock">
            ...
            <div className={`tk-priceΔ num ${mock.chg >= 0 ? "up" : "down"}`}>
              {mock.chg >= 0 ? "▲" : "▼"}{" "}
              ${Math.abs((mock.price * mock.chg) / 100).toFixed(2)}{" "}
              ({mock.chg > 0 ? "+" : ""}{mock.chg.toFixed(2)}%)
            </div>
            <div className="tk-pricemeta num">
              last · 4:00pm ET · prev close ${(mock.price - (mock.price * mock.chg) / 100).toFixed(2)}
            </div>
          </div>
        </div>
        <div className="tk-scoreblock">
          ...
          <span className="lm-sigpill" style={{ alignSelf: "center" }}>
            BUY · LONG
          </span>
      ...
      <section className="mt-pagesection">
        <article className="mt-card">
          <div className="mt-sectionhead" style={{ marginBottom: 16 }}>
            <div>
              ...
              <div className="mt-h2">
                ...
                <span style={{ color: "var(--mt-ink-2)", fontSize: 14, fontFamily: "var(--mt-font-ui)" }}>
                  · {tf} window
                </span>
              </div>
            </div>
          ...
          <BigHistoryChart
            data={series}
            accent={mock.chg >= 0 ? "var(--mt-up)" : "var(--mt-down)"}
            height={320}
          />
      ...
      <section className="mt-pagesection">
        <div className="mt-pillgroup tk-tabs">
          {[
            ...
            <button key={id} className={`mt-pill ${tab === id ? "on" : ""}`} onClick={() => setTab(id)}>
              {l}
            </button>
          ))}
        </div>
        ...
        {tab === "score" && (
          <article className="mt-card mt-fade">
            <div className="mt-eyebrow">
              Composition of <b className="num">{total.toFixed(2)}</b>
              <span className="lm-iddleg-dim" style={{ marginLeft: 6 }}>/10</span>
            </div>
                ...
                {items.map((c) => (
                  <tr key={c.key}>
                    ...
                    <td className="num">
                      ...
                      <span className="lm-scoredim">%</span>
                    </td>
              ...
              <tbody>
                {[
                  ...
                  <tr key={date}>
                    ...
                    <td>{role}</td>
                    <td>
                      <span className={`mt-tag mt-tag--${act === "buy" ? "calm" : "extreme"}`}>
                        {act.toUpperCase()}
                      </span>
                    </td>
                    <td className={`num ${sh >= 0 ? "up" : "down"}`}>
        ...
        {tab === "options" && (
          <article className="mt-card mt-fade">
            ...
            <div className="tk-keygrid" style={{ marginTop: 10 }}>
              {[["Call vol", "12,420"], ["Put vol", "5,180"], ["C/P ratio", "2.40"], ["IV rank", "31"], ["IV (30d)", "42.6%"], ["Skew", "+1.4σ"]].map(([k, v]) => (
                <div key={k} className="tk-kvcell">
                  <div className="mt-eyebrow">{k}</div>
                  <b className="num">{v}</b>
                </div>
        ...
        {tab === "fund" && (
          <article className="mt-card mt-fade">
            ...
            <div className="tk-keygrid" style={{ marginTop: 10 }}>
              {[["Revenue", "$248M"], ["YoY growth", "+18.4%"], ["Gross margin", "68.2%"], ["Op margin", "12.4%"], ["EBITDA", "$42M"], ["FCF", "$28M"], ["Net income", "$14M"], ["Debt/Eq", "0.42"]].map(([k, v]) => (
                <div key={k} className="tk-kvcell">
                  <div className="mt-eyebrow">{k}</div>
                  <b className="num">{v}</b>
                </div>
