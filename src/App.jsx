...
// WATCHLIST_FALLBACK now imported from ./data/watchlistFallback (Phase 4b PR-A).
...
//    to the Scanner tab.
// TickerDetailModal moved to ./components/TickerDetailModal.jsx (Phase 4b PR-A).
...
const navToScannerFor=(ticker)=>{
  // Scanner tab merged into portopps 2026-04-27 — focusTicker plumbing kept
  // for safety though the inline Scanner ignores it.
...
{tab==="home" && (()=>{
  ...
  <main className="fade-in main-padded mt-home" style={{
    ...
    {/* ─── WELCOME HEAD ─ hero + 3 orientation tiles + mission strip ───
         Joe-approved welcome hero (2026-04-27). The dense punchline that
         used to open the page moved to the cards below; the home page now
...
<div style={{background:"var(--surface)",border:"1px solid var(--border-faint)",borderRadius:8,padding:"24px 24px 22px",marginBottom:14}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:24,flexWrap:"wrap",marginBottom:14}}>
<div style={{flex:"1 1 360px",minWidth:0}}>
<div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"var(--accent)",letterSpacing:"0.18em",textTransform:"uppercase",marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
<span style={{display:"inline-block",width:20,height:1,background:"var(--accent)",opacity:0.6}}/>
...
<PositionsTable
  ...
  // 2026-05-04 (Joe directive) — caption describes the trading session the
  // prices represent, not when a script ran. The Yahoo fallback path keeps
  // positions.price refreshed during/after market hours; Polygon publishes
  // today's data at midnight ET so a "4:06 PM" timestamp would lie. The
  // Taxable positions table does not surface events, so eventsCaption is
  // omitted (was previously a confusing extra line).
  priceCaption={(() => {
    const sess = latestTradingSessionDate();
    return `Prices: latest close · ${formatTradingDayLabel(sess)}`;
  })()}
  footnoteSource="Yahoo Finance + Unusual Whales"
/>
</div>
</div>
