...
// Live-grade a Risk Scenario against current indicator readings.
//   armed   — within striking distance of the trigger (within 25% of threshold)
//   active  — trigger has fired
//   dormant — well below threshold, or no live data
function gradeScenario(scn, macroFacts) {
  if (!macroFacts) return { state: "dormant", note: null };
  const rates = macroFacts.real_rates?.current;
  const curve = macroFacts.yield_curve?.current;
  const hyig = macroFacts.hy_ig?.current;
  switch (scn.indicator_id) {
    case "real_rates":
      if (rates == null) return { state: "dormant", note: null };
      if (rates >= 2.0) return { state: "active",  note: `10Y TIPS ${rates.toFixed(2)}% (≥ 2.0% trigger)` };
      if (rates >= 1.5) return { state: "armed",   note: `10Y TIPS ${rates.toFixed(2)}% (approaching 2.0%)` };
      return                   { state: "dormant", note: `10Y TIPS ${rates.toFixed(2)}%` };
    case "hy_ig":
      if (hyig == null) return { state: "dormant", note: null };
      if (hyig >= 250)  return { state: "active",  note: `HY−IG ${hyig.toFixed(0)}bp (≥ 250bp trigger)` };
      if (hyig >= 200)  return { state: "armed",   note: `HY−IG ${hyig.toFixed(0)}bp (approaching 250bp)` };
      return                   { state: "dormant", note: `HY−IG ${hyig.toFixed(0)}bp` };
    case "yield_curve":
      if (curve == null) return { state: "dormant", note: null };
      if (curve <= 25)   return { state: "active",  note: `10Y−2Y ${curve.toFixed(0)}bp (≤ +25bp flat trigger)` };
      if (curve <= 50)   return { state: "armed",   note: `10Y−2Y ${curve.toFixed(0)}bp (flattening toward +25bp)` };
      return                    { state: "dormant", note: `10Y−2Y ${curve.toFixed(0)}bp` };
    default:
      return { state: "dormant", note: null };
  }
}
...
export default function AssetAllocation({ onOpenTicker }) {
  ...
  // Stance pill — reads what the v9 MODEL is doing, not the loose composite
  // classifier. Keeps the pill consistent with hero copy + KPI cards.
  const stance = useMemo(() => {
    if (!alloc) return { label: "—", color: "var(--text-muted)", bg: "var(--surface)" };
    const def = 1 - alloc.equity_share;
    if (def > 0.05)             return { label: "Defensive",  color: "var(--red-text)",    bg: "rgba(255,69,58,0.18)" };
    if (alloc.leverage > 1.05)  return { label: "Aggressive", color: "var(--green-text)",  bg: "rgba(48,209,88,0.18)" };
    return                              { label: "Balanced",   color: "var(--orange-text)", bg: "rgba(255,159,10,0.18)" };
  }, [alloc]);

  // Live rating map. PRIMARY source: alloc.all_industry_groups when present.
  // FALLBACK: derive from picks (any picked ticker → its IG is OW). This avoids
  // showing the SECTOR_IG_MAP hardcoded ratings when v9_allocation.json doesn't
  // include the per-IG ranking yet (current production schema as of 2026-04-28).
  const igRatingMap = useMemo(() => {
    const fromAll = buildIgRatingMap(alloc?.all_industry_groups);
    if (fromAll.size > 0) return fromAll;
    // Derive from picks: each picked ticker gets OW for the IG it represents.
    const map = new Map();
    if (Array.isArray(alloc?.picks)) {
      for (const p of alloc.picks) {
        // Find which IG in SECTOR_IG_MAP carries this ticker
        for (const sector of SECTOR_IG_MAP) {
          for (const g of sector.groups) {
            if (g.ticker === p.ticker) {
              map.set(g.name, { rating: "ow", ticker: p.ticker, sector: sector.sector, indicator_rank: p.indicator_rank, momentum_rank: p.momentum_rank });
            }
          }
        }
      }
    }
    // Everything else falls back to "mw" by default (no UW noise — UWs come
    // from the model when it explicitly underweights, not by silence).
    for (const sector of SECTOR_IG_MAP) {
      for (const g of sector.groups) {
        if (!map.has(g.name)) {
          // If the defensive sleeve is active, the bond-proxy IGs go UW.
          const defensive = (1 - (alloc?.equity_share || 1)) > 0.05;
          const isBondProxy = g.name === "Equity REITs" || g.name === "Utilities" || g.name === "Banks" || g.name === "Pharmaceuticals, Biotech & Life Sciences";
          map.set(g.name, { rating: defensive && !isBondProxy ? "mw" : (isBondProxy ? "uw" : "mw"), ticker: g.ticker });
        }
      }
    }
    return map;
  ...
  const margin = alloc ? Math.max(0, alloc.leverage * alloc.equity_share - 1.0) : 0;
  // Model-state derived stance — sources of truth are equity_share and leverage,
  // not the loose composite-zone classifier. Avoids "all benign / aggressive
  // tilt" copy when the v9 model has actually de-risked (e.g. R&L cliff at 20).
  const modelState = (() => {
    if (!alloc) return { mode: "loading", label: "Loading", lean: "" };
    const tot = alloc.leverage * alloc.equity_share;       // gross deployed
    const lev = alloc.leverage;
    const def = 1 - alloc.equity_share;                    // defensive sleeve fraction
    if (def > 0.05)         return { mode: "derisked",  label: "Capital-preservation mode", lean: `${(alloc.equity_share*100).toFixed(0)}% equities · ${(def*100).toFixed(0)}% defensive sleeve · no leverage` };
    if (lev > 1.05)         return { mode: "leveraged", label: "Risk-on with leverage",     lean: `${(tot*100).toFixed(0)}% equities (${((lev-1)*100).toFixed(0)}% margin) · defensive sleeve off` };
    return                          { mode: "balanced",  label: "Balanced — fully invested",lean: "100% equities, no leverage, defensive sleeve off" };
  })();
  // alloc.alpha from compute_v9_allocation.py is gross deployment (equity_share*leverage), NOT excess return. Read the methodology field instead and convert annualized vs-SPY differential to monthly.
  const monthlyExcessPct = alloc?.methodology?.vs_spy_cagr_diff != null ? (alloc.methodology.vs_spy_cagr_diff * 100 / 12) : null;
  ...
  const heroSubtitle = useMemo(() => {
    if (!macroSnap || !alloc) return "Loading macro snapshot...";
    // FIRST: respect what the model is actually doing. The Today's Macro
    // composite classifier (Quiet/Normal/Elevated/Stressed at -50/50/75)
    // is looser than the v9 leverage cliff at R&L > 20, so the user can
    // see "all benign" while the model has already de-risked. Bridge it.
    if (modelState.mode === "derisked") {
      return `Model has de-risked: equity at ${(alloc.equity_share*100).toFixed(0)}%, ${((1-alloc.equity_share)*100).toFixed(0)}% in the defensive sleeve, no leverage. Lean is capital preservation, not aggression.`;
    }
    if (modelState.mode === "leveraged") {
      return `Risk-on regime: ${(alloc.leverage*100).toFixed(0)}% gross with ${((alloc.leverage-1)*100).toFixed(0)}% margin used. Composites support running at full leverage.`;
    }
    // SECOND fallback: balanced state — use composite classification for color.
    ...
    if (anyStressed) return "One or more composites in the stressed zone. Defensive sleeve will activate at next rebalance.";
    if (elevatedCount >= 2) return "Composites turning elevated. Trim cyclicals; watch stress thresholds.";
    if (elevatedCount === 1) return "One composite in the elevated zone. Stay measured; monitor for further drift.";
    return "Balanced macro read. Allocation fully invested in equities, no leverage.";
  }, [macroSnap, alloc, modelState.mode]);
    ...
    <main style={{ maxWidth: 1440, margin: "0 auto", padding: "var(--space-6) var(--space-8) var(--space-12)" }}>
      ...
      <section style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
          <div style={{ minWidth: 0, flex: "1 1 320px" }}>
            ...
            <h1 style={{ fontFamily: "var(--font-display, Fraunces, Georgia, serif)", fontSize: 32, fontWeight: 400, margin: "0 0 10px", letterSpacing: "-0.012em", lineHeight: 1.1, maxWidth: 720 }}>
              {stance.label} — <em style={{ fontStyle: "italic", color: "var(--accent)" }}>{modelState.mode === "derisked" ? "capital preservation" : modelState.mode === "leveraged" ? "leaning into cyclical rotation" : "balanced positioning"}.</em>
            </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 12 }}>
          ...
          <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "14px 16px" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 26, fontWeight: 500, color: "var(--green-text)", marginTop: 2 }}>{monthlyExcessPct == null ? "—" : (monthlyExcessPct >= 0 ? "+" : "") + monthlyExcessPct.toFixed(2) + "%"}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div style={{ padding: "20px 22px", background: "color-mix(in srgb, var(--green-text) 10%, transparent)", borderLeft: "4px solid var(--green-text)", borderRadius: "var(--radius-md)" }}>
            ...
            <div style={{ fontFamily: "var(--font-display, var(--font-ui))", fontSize: 20, fontWeight: 500, marginBottom: 14, lineHeight: 1.3, color: "var(--text)" }}>
              {modelState.mode === "derisked"
                ? "De-risk: scale equity exposure down, raise the defensive sleeve, drop margin to zero."
                : modelState.mode === "leveraged"
                ? "Get long the cyclical complex. Pull capital out of bond proxies and use the leverage budget."
                : "Stay fully invested in cyclicals; no leverage, no defensive sleeve."}
            </div>
            <ul style={{ fontSize: 14, lineHeight: 1.7, paddingLeft: 18, margin: 0, color: "var(--text-2)" }}>
              {(picks?.length || 0) > 0 && (
                <li><strong>Hold the {picks.length} overweights.</strong> {picks.map((p) => `${p.name} (${p.ticker})`).join(", ")} — equal-weighted at {(((picks[0]?.weight_within_equity) || 0) * 100).toFixed(0)}% of the equity sleeve each.</li>
              )}
              {modelState.mode === "derisked" && (
                <>
                  <li><strong>Defensive sleeve is on.</strong> {((1 - alloc.equity_share) * 100).toFixed(0)}% of capital sits in the defensive blend (BIL, TLT, GLD, LQD per the latest weights). This is the model's automatic capital-preservation response.</li>
                  <li><strong>Cut margin to zero.</strong> The model has dropped leverage to {alloc.leverage.toFixed(2)}×. Pay down any existing margin and stop adding gross.</li>
                  <li><strong>Don't fight the de-risk.</strong> Wait for the R&amp;L composite to cool back below the cliff before re-engaging cyclical leverage.</li>
                </>
              )}
              {modelState.mode === "leveraged" && (
                <>
                  <li><strong>Use leverage to {(alloc.leverage * 100).toFixed(0)}% gross.</strong> Add ~{((alloc.leverage - 1) * 100).toFixed(0)}% via margin or 2× sector ETFs. Calm regimes earn the leverage budget.</li>
              ...
              {modelState.mode === "balanced" && (
                <li><strong>Hold {(alloc.leverage * 100).toFixed(0)}% gross.</strong> Fully invested in the cyclical picks; no margin used; defensive sleeve dormant unless R&amp;L breaks higher.</li>
              )}
              <li><strong>Watch real rates.</strong> If 10Y TIPS breaks above 2%, trim Semis first — long-duration tech compresses fastest.</li>
              <li><strong>Watch the credit-spread trigger.</strong> The HY−IG read in the macro panel below shows current vs. the 250bp cliff. Once it crosses, the model will activate (or expand) the defensive sleeve.</li>
            </ul>
          </div>
      ...
      <section style={{ padding: "var(--space-8) var(--space-10)", background: "var(--surface-solid)", border: "1px solid var(--border-strong)", borderRadius: "var(--radius-lg)", marginBottom: "var(--space-6)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 10 }}>
          ...
          <span style={{ fontSize: 10, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 4 }}>
            {(() => {
              const grades = RISK_SCENARIOS.map((s) => gradeScenario(s, macroFacts).state);
              const fired = grades.filter((g) => g === "active").length;
              const armed = grades.filter((g) => g === "armed").length;
              const dot = fired > 0 ? "var(--red-text)" : armed > 0 ? "var(--orange-text)" : "var(--green-text)";
              return (<>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: dot }}/>
                Stress {fired}/{RISK_SCENARIOS.length} fired{armed > 0 ? ` · ${armed} armed` : ""}
              </>);
            })()}
          </span>
        </div>
        ...
        {RISK_SCENARIOS.map((scn, i) => {
          const grade = gradeScenario(scn, macroFacts);
          const gradeStyle = grade.state === "active"
            ? { bg: "rgba(255,69,58,0.18)", color: "var(--red-text)", label: "Active" }
            : grade.state === "armed"
            ? { bg: "rgba(255,159,10,0.18)", color: "var(--orange-text)", label: "Armed" }
            : { bg: "rgba(120,120,120,0.12)", color: "var(--text-muted)", label: "Dormant" };
          return (
          <div key={i} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "16px 20px", marginTop: 10, background: "var(--bg)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
              <div style={{ fontFamily: "var(--font-display, var(--font-ui))", fontWeight: 500, fontSize: 15, display: "flex", alignItems: "center", gap: 10 }}>
                {scn.trigger}
                <span style={{ fontSize: 10, padding: "3px 9px", borderRadius: 999, background: gradeStyle.bg, color: gradeStyle.color, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>{gradeStyle.label}</span>
              </div>
              {scn.indicator_id && (
            ...
            {grade.note && (
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: gradeStyle.color, marginTop: 6 }}>Live: {grade.note}</div>
            )}
