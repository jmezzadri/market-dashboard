...
export default function AssetTilt({ onOpenTicker }) {
  ...
  // SINGLE SOURCE OF TRUTH for the engine's headline backtest statistics.
  // Bug #1207: the hero intro bullet, the "Key Statistics" box and the
  // "Backtest Validation" panel previously disagreed — two surfaces showed
  // hand-typed stale numbers (and read the wrong strategy key `engine`),
  // one read the live computed result. All three now derive from the SAME
  // live source the Backtest Validation panel uses: the `asset_tilt`
  // strategy (the full Asset Tilt recommendation this page actually sells —
  // the strategy carrying the "MacroTilt" badge in that panel) benchmarked
  // against `spy`. Numbers come from /macrotilt_engine_backtest.json,
  // regenerated by the calibrated engine; no values are hard-coded.
  const headlineStats = (() => {
    const at = backtest?.validation?.asset_tilt;
    const spy = backtest?.validation?.spy;
    const nWeeks = backtest?.validation?.n_weeks;
    if (!at || !spy) return null;
    return {
      engineCagr: at.cagr, spyCagr: spy.cagr,
      engineSharpe: at.sharpe, spySharpe: spy.sharpe,
      engineMaxDD: at.max_drawdown, spyMaxDD: spy.max_drawdown,
      nWeeks,
    };
  })();
    ...
    <main style={{ maxWidth: 1280, margin: "0 auto", padding: "0 0 48px" }}>
      ...
      <PageHero
        ...
        bullets={[
          ...
          headlineStats
            ? `Validated 1986 → 2026 over ${(headlineStats.nWeeks || 0).toLocaleString()} weeks · Sharpe ${headlineStats.engineSharpe.toFixed(2)} vs SPY ${headlineStats.spySharpe.toFixed(2)} · max drawdown ${Math.abs(headlineStats.engineMaxDD * 100).toFixed(0)}% vs ${Math.abs(headlineStats.spyMaxDD * 100).toFixed(0)}%`
            : "Validated 1986 → 2026 — Sharpe and drawdown statistics load from the latest backtest",
        ]}
        right={
<aside style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "18px 20px 14px", display: "flex", flexDirection: "column", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, flex: 1, textAlign: "left" }}>
            {[
              { label: "CAGR",         value: headlineStats ? headlineStats.engineCagr.toFixed(2) + "%" : "—",  sub: headlineStats ? "vs SPY " + headlineStats.spyCagr.toFixed(2) + "%" : "" },
              { label: "Sharpe",       value: headlineStats ? headlineStats.engineSharpe.toFixed(2) : "—",      sub: headlineStats ? "vs SPY " + headlineStats.spySharpe.toFixed(2) : "" },
              { label: "Max Drawdown", value: headlineStats ? (headlineStats.engineMaxDD * 100).toFixed(1) + "%" : "—", sub: headlineStats ? "vs SPY " + (headlineStats.spyMaxDD * 100).toFixed(1) + "%" : "" },
              { label: "Validated",    value: "1986 → 2026", sub: (headlineStats?.nWeeks ? headlineStats.nWeeks.toLocaleString() : "—") + " weeks" },
            ].map(t => (
