...
5. **Dated 2026-09-10 — the rule extends to ANY "click", not just GitHub.** Joe: *"What do you want me to click!? You keep saying click something and nothing pops up to click!! This is the 10th time you've asked me!"* Twice in one session the morning-brief agent closed with an action for him: "approve the pending scheduled-task update in your app" (an MCP approval that never surfaced as anything he could see) and "close PR #1653 whenever you're next in GitHub". **Before writing any sentence that asks Joe to click, approve, confirm or open something, check two things:** (a) does the thing actually appear in his interface — a tool call that returns "requires approval" is NOT a visible prompt, and scheduled tasks are not visible on mobile at all; (b) is it still necessary after the work just done — both asks here were already moot, because the repo fix had superseded the task edit and the stray PR contained no changes. An ask that is invisible, unnecessary, or both is worse than silence: he cannot act on it, and it teaches him that the agent's status reports contain phantom homework.
6. **When something genuinely cannot be done from here, say what it is and that it needs nothing — do not convert it into a task for Joe.** The agent-write path can create and merge but cannot close a pull request. The correct report is "PR #1653 is empty and inert, ignore it", never "close it next time you're in GitHub".

7. **Dated 2026-09-14 — fifth repeat. "The fix is correct but code changes wait for your click" is this rule, restated as a policy the agent invented.** The morning-brief run found the page-render helper broken (it required `playwright`, which this repo does not install — package.json declares `playwright-core`), fixed it properly in the repo rather than patching its own sandbox — all correct — and then closed with *"one approval needed: PR #1662 is open and unmerged... code changes to the live branch wait for your click."* No such policy exists. Joe: *"Why did I just get this error?"* **A pull request the agent opened is the agent's to merge, in the same run, without exception.** The merge path is the ops-code-commit function: `{"branch":...,"merge":true,"files":[...]}` to commit and merge in one call, or `{"merge_pr_number": N}` to merge one already open — it already retries through the "base branch was modified" race, so a busy main branch is not a reason to stop. Validate before shipping (`node --check` for JavaScript — the check that would have caught this class of bug at authoring time). If a merge truly fails after retries, report in plain English that it could not ship and why; name no branch, PR number or URL, and do not hand the merge to Joe. Self-check before sending anything to Joe: if the sentence contains "approval", "merge", "PR", "branch" or a github.com link, it is wrong — delete it and go do the work.
...
3. **An event the external sweep finds that the site's own calendar missed is a data ticket filed the same run** (4.30: anything dated that renders is a feed). Today's gap: the annual payrolls benchmark revision — a market mover for a live call, absent from the release calendar feed.
   *2026-09-10:* second ticket of this class (#1254): the calendar was US-only, so the ECB decision that day — direct event risk to the live EUR/USD call — was absent. Fixed same day (PR #1658): the calendar now carries ECB / BoE / BoJ decision dates from each bank's own published schedule, tier 1, same authority pattern as the FOMC block. The rule worked: the sweep filed it, the sweep fixed it.
