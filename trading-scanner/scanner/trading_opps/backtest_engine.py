...
def _pull_insider():
    ...
    while True:
        ...
        # 2026-07-20 CUTOVER: SEC EDGAR is the production insider table.
        # INSIDER_TABLE env override remains for the parity harness, which
        # compares against the legacy UW table until it lapses 2026-08-12.
        _tbl = os.environ.get("INSIDER_TABLE", "insider_history_edgar")
...
def main():
    ...
    calibrated = {
        "_comment": "MacroTilt Trading Opportunities screener — calibrated "
                    "config. The live engine (run_screener.py) reads this file. "
                    "Phase 2 retail-optimized 2026-05-20. 2026-05-21: dark-pool "
                    "and options scoring layers activated — score ceiling 5 -> "
                    "10. Insider points / trend / window / launch threshold are "
                    "backtest-calibrated and regenerated here; dark-pool and "
                    "options point values are from the locked spec and are NOT "
                    "backtested (see their _note fields). Authoritative dark-"
                    "pool/options thresholds live in darkpool_scoring.py and "
                    "options_scoring.py.",
        "version": "phase2-retail-2.0",
        "scoring_version": "2026-05-21-darkpool-options",
        "scoring_updated": "2026-05-21",
        "generated": str(datetime.now().date()),
        "scoring_mode": "insider + trend + dark_pool + options (long side)",
        "shadow_layers": [],
        "_shadow_layers_note": "Empty since 2026-05-21 — dark-pool and options "
                               "are live. They enrich the score and ranking but "
                               "never change the launch decision; the launch "
                               "gate is insider + trend only.",
        "universe": {
        ...
        "long": {
            ...
            "dark_pool": {
                "band_active_points": 2,
                "single_block_points": 1,
                "layer_cap": 2,
                "window_hours": 72,
                "band_halfwidth_pct": 0.015,
                "cluster_concentration": 0.60,
                "min_window_premium_usd": 1000000.0,
                "single_block_mult": 3.0,
                "single_block_min_premium_usd": 500000.0,
                "backtested": False,
                "_note": "Activated 2026-05-21. Point values from locked spec "
                         "Section 6; engine thresholds are Senior-Quant design. "
                         "NOT backtested. Owner-approved to ship with an "
                         "on-page disclaimer. Authoritative values live in "
                         "darkpool_scoring.py.",
            },
            "options": {
                "strong_tier_points": 4,
                "base_tier_points": 3,
                "layer_cap": 4,
                "dte_min": 14,
                "dte_max": 45,
                "moneyness_min": 1.00,
                "moneyness_max": 1.15,
                "at_ask_threshold": 0.65,
                "vol_oi_strong": 3.0,
                "vol_oi_base": 1.0,
                "min_prev_oi": 100,
                "backtested": False,
                "_note": "Activated 2026-05-21. Point values from locked spec "
                         "Section 7. 'Moderately out-of-the-money' filter "
                         "switched from the spec's CANDIDATE delta 0.35-0.50 "
                         "to a strike/price moneyness band because stored "
                         "option delta is ~68% null in production. NOT "
                         "backtested. Authoritative values live in "
                         "options_scoring.py.",
            },
            "launch_threshold": threshold,
            "_launch_threshold_note": "The launch gate is the insider + trend "
                                      "score only; dark-pool and options "
                                      "points are added on top toward the "
                                      "ceiling of 10 but never add or drop a "
                                      "name from the list.",
            "max_attainable_score": 10,
        },
        "short": {
            "trend": {
