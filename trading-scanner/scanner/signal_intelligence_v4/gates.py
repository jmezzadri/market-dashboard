...
def insider_gate_passes(
    ...
    Args:
        ...
        insider_history: ALL insider events for this ticker. Each row:
            {"date": date, "owner": str, "transaction_code": str (e.g. 'P', 'S', 'A'),
             "amount": int (shares), "stock_price": float, "is_officer": bool}
            Must include events going back at least `insider_window_days +
            first_buy_lookback_days` days for the first-buy check to be valid.
        require_first_buy: if True (v4.1 default), require ≥1 first-buyer.
            If False, falls back to v4.0 (any P-buy in window).
    ...
    Returns:
        {
            ...
            "unique_p_buyers": int,
            "first_buyers": list[str],   # names of first-buy buyers
            "total_dollar": float,        # tiebreaker
        }
    """
...
def apply_gates(
    ...
    g1 = insider_gate_passes(score_date, insider_history, require_first_buy)
