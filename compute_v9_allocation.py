...
def main():
    ...
    for tkr in EQUITY:
        ...
        f = fit_per_asset_forecast(win[tkr], wf, per_bucket_factors(EQUITY[tkr]["name"]))
        if f is not None: mu[tkr] = f
    for tkr in DEFENSIVE:
        if tkr not in win.columns or win[tkr].dropna().shape[0] < 24: continue
        f = fit_per_asset_forecast(win[tkr], wf, DEFENSIVE_FACTORS[tkr])
        if f is not None: mu[tkr] = f

    avail_e = [t for t in EQUITY if t in mu]
    avail_d = [t for t in DEFENSIVE if t in mu]
    print(f"    forecasts ready: {len(avail_e)} equity, {len(avail_d)} defensive")
