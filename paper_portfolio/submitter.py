...
# Submission loop
#
# 2026-08-11 — the Conviction Events kill-switch entry refusal that used to
# live here is GONE (Joe: "no point freezing new buys just because existing
# names are losing"). The kill switch is a MONITOR computed in
# paper_portfolio.conviction's post-close phase; it gates nothing, and the
# submitter no longer reads ce_kill_switch at all. Per-position loss control
# is the -15% catastrophe stop, which reaches this submitter as an ordinary
# market-on-open SELL. Do not reintroduce a refusal branch here.
# ─────────────────────────────────────────────────────────────────────────────
