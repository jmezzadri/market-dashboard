...
ism:{mean:52,sd:5.5,dir:"lw"},copper_gold:{mean:0.24,sd:0.05,dir:"lw"},
bkx_spx:{mean:0.44,sd:0.07,dir:"lw"},bank_unreal:{mean:5,sd:8,dir:"hw"},
...
const AS_OF={
vix:"Apr 06 2026",hy_ig:"Apr 01 2026",eq_cr_corr:"Apr 02 2026",
yield_curve:"Apr 04 2026",move:"Apr 02 2026",anfci:"Mar 27 2026",
stlfsi:"Mar 27 2026",real_rates:"Apr 02 2026",sloos_ci:"Q1 2026",
cape:"Mar 2026",ism:"Mar 2026",copper_gold:"Apr 02 2026",
bkx_spx:"Apr 02 2026",bank_unreal:"Q4 2025",credit_3y:"Feb 2026",
term_premium:"Apr 02 2026",cmdi:"Mar 2026",loan_syn:"Apr 01 2026",
usd:"Apr 02 2026",cpff:"Apr 02 2026",skew:"Apr 02 2026",
sloos_cre:"Q1 2026",bank_credit:"Mar 25 2026",jobless:"Apr 03 2026",
jolts_quits:"Jan 2026",
};

const IND={
vix:["VIX","Equity Volatility","equity",1,"index",1,24.5,23.9,17.2,19.5,15.0,false,
...
hy_ig:["HY–IG Spread","Credit Risk Premium","credit",1,"bps",0,228,268,245,280,220,false,
...
eq_cr_corr:["EQ–Credit Corr","Risk-Off Synchronization","equity",1,"corr",2,0.74,0.61,0.55,0.50,0.40,false,
...
yield_curve:["10Y–2Y Slope","Yield Curve","rates",1,"bps",0,51,52,35,15,-20,false,
...
move:["MOVE Index","Rates Volatility","rates",2,"index",0,112,98,95,90,85,false,
...
anfci:["ANFCI","Chicago Fed Fin. Conditions","fincond",2,"z-score",2,0.12,0.08,0.06,0.04,-0.05,false,
...
stlfsi:["STLFSI","St. Louis Fed Stress Index","fincond",2,"index",2,0.31,0.22,0.18,0.12,-0.10,false,
...
real_rates:["10Y TIPS","Real Interest Rates","rates",2,"%",2,1.82,1.90,1.75,1.85,1.50,false,
...
sloos_ci:["SLOOS C&I","Business Lending Standards","bank",2,"%",1,14.2,9.8,8.0,6.0,2.0,false,
...
copper_gold:["Copper/Gold Ratio","Real Economy vs. Safe Haven","economy",2,"ratio",3,0.182,0.195,0.210,0.225,0.240,true,
...
bkx_spx:["BKX/SPX Ratio","Bank vs. Market Strength","bank",2,"ratio",3,0.335,0.352,0.365,0.378,0.395,true,
...
credit_3y:["3Y Credit Growth","3-Year Bank Credit Expansion","bank",2,"% 3yr",1,11.2,11.8,12.5,13.2,12.8,true,
...
term_premium:["Kim–Wright 10Y","10-Year Term Premium","rates",3,"bps",0,72,55,45,35,20,false,
...
cmdi:["CMDI","Corp Bond Market Distress","credit",3,"index",2,0.42,0.38,0.30,0.25,0.12,false,
...
loan_syn:["HY Eff. Yield","High Yield Effective Yield","credit",3,"%",2,7.84,7.45,7.0,6.5,6.2,false,
...
usd:["USD Index","Trade-Weighted Dollar","fincond",3,"index",1,104.2,103.1,102.5,101.8,101.0,false,
...
cpff:["USD Funding","3M CP vs. Fed Funds Spread","fincond",3,"bps",0,18,14,12,10,8,false,
...
skew:["SKEW Index","Options-Implied Tail Risk","equity",3,"index",0,148,141,138,135,130,false,
...
sloos_cre:["SLOOS CRE","CRE Lending Standards","bank",3,"%",1,21.5,18.3,15.0,12.0,8.0,false,
...
bank_credit:["Bank Credit","YoY Bank Credit Growth","bank",3,"% YoY",1,2.8,3.4,3.8,4.2,5.0,false,
...
jobless:["Init. Claims","Weekly Jobless Claims","labor",3,"K",0,202,224,215,210,208,false,
...
jolts_quits:["JOLTS Quits","Voluntary Quit Rate","labor",3,"%",1,2.1,2.3,2.35,2.45,2.55,false,
...
return              {label:"Stable",       arrow:"→", col:"#888"};
}
...
return{arrow:"→",col:"#888",label:"Stable"};
}
...
{label:"SVB",year:"23Q1",color:"#eab308"},
];
...
{ticker:"CASH", name:"Cash & Sweep Funds",         value:21800, price:1,     shares:21800,sector:"Cash",     beta:0.00,color:"#555",
...
{ticker:"SPAXX",name:"Fidelity Govt Money Market",   value:7400, price:1.00,  shares:7400,sector:"Cash",     beta:0.00,color:"#555",
...
{ticker:"FDRXX",name:"Fidelity Govt Money Market", value:4800, price:1.00,  shares:4800,sector:"Cash",   beta:0.00,color:"#555",
...
<text x={pL-3} y={pT+4} textAnchor="end" fill="#333" fontSize="6" fontFamily="monospace">{fmtFn(dir==="lw"||dir==="nw"?mn:mx)}</text>
<text x={pL-3} y={pT+IH+2} textAnchor="end" fill="#333" fontSize="6" fontFamily="monospace">{fmtFn(dir==="lw"||dir==="nw"?mx:mn)}</text>
{marks.map(cm=>(
<g key={cm.label}>
...
<path d={fullPath} fill="none" stroke="#2a2a2a" strokeWidth="1.5" strokeLinejoin="round"/>
...
<text key={i} x={xp(i)} y={H-4} textAnchor="middle" fill="#2a2a2a" fontSize="6" fontFamily="monospace">{l}</text>
))}
{hover&&(
<g>
...
<text x={ttX} y={ttY+7} textAnchor="middle" fill="#666" fontSize="5.5" fontFamily="monospace">{hover.label}</text>
</g>
)}
</svg>
);
...
const data=HIST[id];if(!data)return null;
const sd=SD[id];
...
<div style={{fontSize:8,color:"#444",fontFamily:"monospace",marginBottom:4}}>
LONG-TERM HISTORY
<span style={{color:"rgba(34,197,94,0.7)",marginLeft:6}}>▬ normal range</span>
{CRISIS.map(cm=>{
const idx=labels.findIndex(l=>l===cm.year||l.startsWith(cm.year));
if(idx<0)return null;
return <span key={cm.label} style={{color:cm.color,marginLeft:8}}>│{cm.label}: {fmt(HIST[id][idx][1])}</span>;
})}
<span style={{color:"#333",marginLeft:8}}>· tap to read values</span>
</div>
<ChartCore data={data} labels={labels} dir={sd?.dir} sdP={sd} crisisData={CRISIS}
...
<g key={v}>
<line x1={pL-2} y1={yp(v)} x2={pL} y2={yp(v)} stroke="#2a2a2a" strokeWidth="0.5"/>
<text x={pL-4} y={yp(v)+3} textAnchor="end" fill="#2a2a2a" fontSize="6" fontFamily="monospace">{v}</text>
</g>
))}
{marks.map(cm=>(
<g key={cm.label}>
...
<text x={cx} y={116} textAnchor="middle" fill="#444" fontSize="7" fontFamily="monospace">COMPOSITE STRESS</text>
</svg>
{onClick&&<div style={{fontSize:7,color:"#333",fontFamily:"monospace",marginTop:1}}>tap for history</div>}
</div>
);
}
...
const vals=[d[11],d[10],d[9],d[8],d[7],d[6]];
const valid=vals.filter(v=>v!=null);
if(!valid.length)return null;
const mn=Math.min(...valid),mx=Math.max(...valid),rng=mx-mn||1;
const dir=SD[id]?.dir;
const W=70,H=22,pd=2;
const pts=vals.map((v,i)=>{
if(v==null)return null;
const x=pd+(i/(vals.length-1))*(W-pd*2);
const raw=(v-mn)/rng;
const y=pd+(dir==="lw"||dir==="nw"?raw:1-raw)*(H-pd*2);
return[x,y];
});
const vp=pts.filter(Boolean);
const spath=makePath(vp);
const col=sdColor(sdScore(id,d[6]));
const last=vp[vp.length-1];
return(
<svg width={W} height={H}>
<path d={spath} fill="none" stroke="#222" strokeWidth="1.5" strokeLinejoin="round"/>
{last&&<circle cx={last[0]} cy={last[1]} r={2.5} fill={col}/>}
</svg>
);
}
...
const catCol=CATS[cat]?.color||"#888";
...
<span style={{fontSize:11,fontWeight:700,color:"#ddd",fontFamily:"monospace"}}>{label}</span>
...
<div style={{fontSize:9,color:"#3a3a3a",marginLeft:9}}>{sub}</div>
<div style={{fontSize:7,color:"#252525",marginLeft:9,fontFamily:"monospace"}}>{AS_OF[id]}</div>
</div>
...
<div style={{height:4,background:"#1a1a1a",borderRadius:2,overflow:"hidden",marginTop:3}}>
<div style={{width:`${Math.max(0,Math.min(100,((s??0)+2)/5*100))}%`,height:"100%",background:col,borderRadius:2}}/>
</div>
</div>
{delta!=null&&(
<div style={{display:"flex",alignItems:"center",gap:6}}>
<span style={{fontSize:9,color:delta>0.1?col:delta<-0.1?"#22c55e":"#555",fontFamily:"monospace"}}>
{delta>0.1?"▲ Worsening":delta<-0.1?"▼ Improving":"→ Stable"} vs {trendPeriod}
</span>
<span style={{fontSize:8,padding:"1px 5px",borderRadius:2,background:(delta>0.1?col:"#22c55e")+"18",color:delta>0.1?col:delta<-0.1?"#22c55e":"#555",fontFamily:"monospace"}}>
...
<div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:"#333",fontFamily:"monospace",marginBottom:4}}>
...
<div style={{fontSize:8,color:"#383838",fontFamily:"monospace",display:"flex",gap:12}}>
...
<div style={{fontSize:8,color:"#555",fontFamily:"monospace",letterSpacing:"0.1em",marginBottom:5}}>WHAT IS THIS INDICATOR?</div>
<div style={{fontSize:9,color:"#666",lineHeight:1.75}}>{desc}</div>
</div>
...
<div style={{fontSize:9,color:"#666",lineHeight:1.75}}>{signal}</div>
</div>
</div>
)}
</div>
...
<div>
<div style={{fontSize:10,fontWeight:700,color:"#ddd",fontFamily:"monospace"}}>{p.ticker}</div>
<div style={{fontSize:8,color:"#3a3a3a"}}>{p.name}</div>
</div>
</div>
<div style={{textAlign:"right"}}>
<div style={{fontSize:12,fontWeight:800,color:"#e0e0e0",fontFamily:"monospace"}}>${p.value.toLocaleString()}</div>
<div style={{fontSize:8,color:"#555",fontFamily:"monospace"}}>{pct}% of acct</div>
</div>
</div>
...
<div key={l}>
<div style={{fontSize:6,color:"#333",fontFamily:"monospace"}}>{l}</div>
<div style={{fontSize:8,color:c||"#888",fontFamily:"monospace",fontWeight:700}}>{v}</div>
</div>
))}
</div>
...
<div style={{fontSize:9,color:"#666",lineHeight:1.75}}>{p.analysis}</div>
</div>
)}
</div>
);
...
<span style={{fontSize:11,fontWeight:700,color:"#ddd",fontFamily:"monospace"}}>{acct.label}</span>
<span style={{fontSize:8,color:"#444",fontFamily:"monospace"}}>{acct.sub}</span>
</div>
<div style={{fontSize:8,color:"#444",marginLeft:16,lineHeight:1.5}}>{acct.note}</div>
</div>
<div style={{textAlign:"right",flexShrink:0,marginLeft:10}}>
<div style={{fontSize:14,fontWeight:800,color:"#e0e0e0",fontFamily:"monospace"}}>${total.toLocaleString()}</div>
<div style={{fontSize:8,color:"#555",fontFamily:"monospace"}}>{pctOfTotal}% of wealth</div>
<div style={{fontSize:9,color:"#444"}}>{open?"▲":"▼"}</div>
</div>
</div>
...
if(score>0.40)return{label:"NEUTRAL",    color:"#888",   short:"=" };
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
  <div style={{flex:1,minWidth:0}}>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
      ...
      <span style={{fontSize:13,fontWeight:800,color:"#ddd",fontFamily:"monospace"}}>{sector.name}</span>
      <span style={{fontSize:8,color:"#444",fontFamily:"monospace"}}>β {sector.beta.toFixed(2)}</span>
    </div>
    <div style={{fontSize:9,color:"#3a3a3a",marginLeft:11}}>{sector.sub}</div>
  </div>
  <div style={{textAlign:"right",flexShrink:0,marginLeft:12}}>
    ...
    <div style={{fontSize:7,color:"#444",fontFamily:"monospace",marginTop:3}}>Rank {rank}/{totalSectors}</div>
  </div>
</div>
...
<div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}}>
  {topHeadwinds.map(h=>{
    const col=h.impact>1.2?"#ef4444":h.impact>0.5?"#f97316":"#555";
...
<div style={{marginTop:12,paddingTop:12,borderTop:"1px solid #1a1a1a"}} onClick={e=>e.stopPropagation()}>
  <div style={{fontSize:8,color:"#555",fontFamily:"monospace",letterSpacing:"0.12em",marginBottom:8}}>SUBSECTOR OUTLOOK · scores from live indicator data</div>
      ...
      return(
        <div key={i} style={{background:"#0a0a0a",border:`1px solid ${ol.color}22`,borderRadius:5,padding:"8px 10px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
            <span style={{fontSize:9,fontWeight:700,color:"#ccc",fontFamily:"monospace"}}>{ss.name}</span>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:5}}>
            {Object.entries(ss.sensitivities).filter(([k,w])=>w!==0).map(([k,w])=>{
              ...
              const col=!isHead?"#22c55e":impact>0.8?"#ef4444":impact>0.3?"#f97316":"#555";
          ...
          <div style={{fontSize:9,color:"#555",lineHeight:1.65}}>{ss.note}</div>
        </div>
      );
    })}
  </div>
  <div style={{background:"#0a0a0a",border:"1px solid #1a1a1a",borderRadius:5,padding:"8px 10px"}}>
    <div style={{fontSize:8,color:"#444",fontFamily:"monospace",marginBottom:5}}>SUPPLY CHAIN & CORRELATION LINKS</div>
    {sector.chainLinks.map((link,i)=>(
      <div key={i} style={{display:"flex",gap:6,marginBottom:3}}>
        <span style={{color:"#2a2a2a",fontSize:9,flexShrink:0}}>→</span>
        <span style={{fontSize:9,color:"#555",lineHeight:1.6}}>{link}</span>
      </div>
    ))}
  </div>
</div>
...
<div style={{padding:"14px 20px",display:"flex",flexDirection:"column",gap:14}}>
  <div style={{background:"#0c0c0c",border:`1px solid ${CONV.color}33`,borderRadius:8,padding:"12px 16px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
      <div>
        ...
        <div style={{fontSize:9,color:"#555",maxWidth:420,lineHeight:1.6}}>Sector scores driven by live indicator data. Parent score = average of dynamic subsector scores. Each subsector has its own macro sensitivity weights.</div>
      </div>
      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
        <div style={{fontSize:7,color:"#444",fontFamily:"monospace",alignSelf:"center"}}>TOP HEADWINDS:</div>
        {topFactors.map(f=>{const col=f.stress>1.2?"#ef4444":f.stress>0.5?"#f97316":"#555";return(
  ...
  <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
    <span style={{fontSize:8,color:"#444",fontFamily:"monospace"}}>FILTER:</span>
    {[{id:null,label:"ALL"},{id:"ow",label:"↑ OW"},{id:"neutral",label:"= NEUTRAL"},{id:"uw",label:"↓ UW"}].map(({id,label})=>(
      <button key={String(id)} onClick={()=>setFilterOutlook(id)}
        style={{padding:"4px 12px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:8,fontFamily:"monospace",background:filterOutlook===id?"#e0e0e0":"transparent",color:filterOutlook===id?"#000":"#555",borderColor:filterOutlook===id?"#e0e0e0":"#1e1e1e"}}>{label}</button>
    ))}
    <span style={{fontSize:8,color:"#333",fontFamily:"monospace",marginLeft:"auto"}}>tap any card to expand</span>
  </div>
  <div style={{background:"#0c0c0c",border:"1px solid #181818",borderRadius:8,padding:"12px 14px"}}>
    <div style={{fontSize:8,color:"#555",fontFamily:"monospace",letterSpacing:"0.1em",marginBottom:8}}>RELATIVE RANKING · bar = macro-adjusted favorability score</div>
      ...
      {scored.map((s,i)=>{
        ...
        return(
          <div key={s.id} style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:8,color:"#333",fontFamily:"monospace",minWidth:16,textAlign:"right"}}>{i+1}</span>
            <span style={{fontSize:9,color:"#777",fontFamily:"monospace",minWidth:160}}>{s.name}</span>
    ...
    <div style={{fontSize:7,color:"#2a2a2a",fontFamily:"monospace",marginTop:6}}>
  ...
  <div style={{fontSize:7,color:"#1e1e1e",fontFamily:"monospace",textAlign:"center"}}>Sector scores from live dashboard indicators · Subsector scores are the source of truth · Not investment advice</div>
</div>
);
}
...
const rollupColors={"Index Funds":"#4a6fa5","Intl Equity":"#6366f1","Individual Stocks":"#f97316","Precious Metals":"#eab308","Crypto":"#a855f7","Cash":"#555"};
...
<div style={{fontSize:8,letterSpacing:"0.25em",color:"#252525",fontFamily:"monospace",marginBottom:4}}>MACRO STRESS MONITOR · Apr 09, 2026</div>
<div style={{fontSize:18,fontWeight:800,letterSpacing:"-0.02em"}}>Market Stress <span style={{color:CONV.color}}>Dashboard</span></div>
<div style={{fontSize:9,color:"#252525",marginTop:2,fontFamily:"monospace"}}>25 indicators · statistically calibrated · 6 accounts</div>
...
<span style={{fontSize:8,color:CONV.level===c.level?c.color:"#444",fontFamily:"monospace",fontWeight:CONV.level===c.level?700:400}}>{c.label}</span>
</div>
))}
</div>
</div>
...
<div style={{fontSize:8,color:"#444",fontFamily:"monospace",cursor:"pointer"}} onClick={()=>setShowCompHist(false)}>✕ close</div>
</div>
<CompHistChart/>
</div>
</div>
...
<div style={{display:"flex",gap:6,alignItems:"center"}}>
  {[["1M",COMP1M100],["3M",COMP3M100]].map(([lbl,val])=>{
    ...
    return <div key={lbl} style={{fontSize:8,color:"#444",fontFamily:"monospace",background:"#111",borderRadius:4,padding:"3px 8px"}}>
      {lbl}: <span style={{color:t.col}}>{val} {t.arrow}</span>
    </div>;
  })}
</div>
<span style={{fontSize:9,color:"#555",fontFamily:"monospace",marginLeft:"auto"}}>→ {CONV.action}</span>
</div>
...
<button key={id} onClick={()=>setTab(id)} style={{padding:"4px 12px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:8,fontFamily:"monospace",background:tab===id?"#e0e0e0":"transparent",color:tab===id?"#000":"#444",borderColor:tab===id?"#e0e0e0":"#1e1e1e"}}>{label}</button>
))}
</div>
...
<div style={{fontSize:7,color:"#444",fontFamily:"monospace",alignSelf:"center",marginRight:4}}>TREND:</div>
...
<div style={{fontSize:7,color:"#444",fontFamily:"monospace",marginBottom:3}}>{lbl}</div>
...
{CONVICTION.map(c=>(<div key={c.level} style={{fontSize:6,color:CONV.level===c.level?c.color:"#333",fontFamily:"monospace",textAlign:"center",flex:1}}>{c.label}</div>))}
</div>
</div>
<div style={{fontSize:8,color:"#555",marginTop:8,fontStyle:"italic"}}>{CONV.action}</div>
</div>

<div style={{background:"#0c0c0c",border:"1px solid #1e1e1e",borderRadius:8,padding:"14px 16px"}}>
<div style={{fontSize:8,color:"#888",fontFamily:"monospace",letterSpacing:"0.15em",marginBottom:2}}>CONVICTION FRAMEWORK</div>
<div style={{fontSize:9,color:"#555",marginBottom:10}}>4 stress regimes · Normal is the market baseline where most time is spent · Extreme reserved for historical crises</div>
...
<div style={{fontSize:11,fontWeight:800,color:CONV.level===c.level?c.color:"#888",fontFamily:"monospace"}}>{c.label}</div>
</div>
<div style={{flex:1,fontSize:9,color:"#555"}}>{c.action}</div>
...
<div style={{fontSize:6,color:"#444",fontFamily:"monospace"}}>{lbl}</div>
...
<div style={{fontSize:7,color:"#333",marginTop:8,fontFamily:"monospace"}}>EQ = Equities · BD = Bonds · CA = Cash · AU = Gold/Commodities · Not financial advice</div>
</div>

<div>
<div style={{fontSize:8,color:"#888",fontFamily:"monospace",letterSpacing:"0.1em",marginBottom:4}}>CATEGORY COMPOSITES</div>
<div style={{fontSize:8,color:"#444",marginBottom:8}}>Stress score per indicator category · tap any card to see constituent indicators</div>
...
<span style={{fontSize:7,color:"#444",fontFamily:"monospace"}}>{lbl}</span>
...
<span style={{fontSize:8,color:"#555",fontFamily:"monospace",minWidth:90,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{label}</span>
...
{scored.length>3&&<div style={{fontSize:7,color:"#2a2a2a",fontFamily:"monospace"}}>+{scored.length-3} more →</div>}
</div>
</div>
);
})}
...
<div style={{fontSize:7,color:"#444"}}>Ranked by SD score · tap to drill in</div>
</div>
<div style={{display:"flex",flexDirection:"column",gap:6}}>
...
<span style={{fontSize:9,color:"#888",fontFamily:"monospace",minWidth:120}}>{label}</span>
...
<div>
<div style={{fontSize:8,color:ACCENT,fontFamily:"monospace",letterSpacing:"0.18em",marginBottom:3}}>AI NARRATIVE ANALYSIS</div>
<div style={{fontSize:10,color:"#444"}}>Cross-indicator synthesis · Unusual correlations · Portfolio implications</div>
</div>
<button onClick={()=>setShowAnalysis(s=>!s)} style={{padding:"7px 18px",borderRadius:4,border:`1px solid ${ACCENT}`,background:ACCENT+"20",color:ACCENT,cursor:"pointer",fontSize:9,fontFamily:"monospace"}}>
{showAnalysis?"▲ HIDE":"▶ SHOW ANALYSIS"}
</button>
</div>
<div style={{padding:"8px 10px",background:"#0a0a0a",border:"1px solid #1e1e1e",borderRadius:4,marginBottom:showAnalysis?12:0}}>
<div style={{fontSize:8,color:"#555",fontFamily:"monospace"}}>📅 Generated Apr 8, 2026 · Post-Iran ceasefire · Based on current readings</div>
</div>
{showAnalysis&&(
<div style={{fontSize:10,color:"#777",lineHeight:1.9}}>
{ANALYSIS.split("\n\n").map((para,i)=>{
const lines=para.split("\n");
const isH=lines[0]===lines[0].toUpperCase()&&lines[0].length<60&&!lines[0].includes(".");
return(<div key={i} style={{marginBottom:14}}>
{isH?(<><div style={{fontSize:8,color:ACCENT,fontFamily:"monospace",letterSpacing:"0.15em",marginBottom:6}}>{lines[0]}</div>
<div style={{color:"#777",lineHeight:1.85}}>{lines.slice(1).join(" ")}</div></>)
:<div style={{color:"#777",lineHeight:1.85}}>{para}</div>}
</div>);
})}
</div>
)}
...
<span style={{fontSize:8,color:"#444",fontFamily:"monospace"}}>TREND:</span>
{["1M","3M","6M","12M"].map(p=>(
<button key={p} onClick={()=>setTrendPeriod(p)} style={{padding:"3px 10px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:9,fontFamily:"monospace",background:trendPeriod===p?"#e0e0e0":"transparent",color:trendPeriod===p?"#000":"#555",borderColor:trendPeriod===p?"#e0e0e0":"#1e1e1e"}}>{p}</button>
))}
</div>
<div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
<button onClick={()=>{setNewOnly(n=>!n);setCatFilter(null);}} style={{padding:"4px 12px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:8,fontFamily:"monospace",background:newOnly?"#eab30822":"transparent",color:newOnly?"#eab308":"#555",borderColor:newOnly?"#eab308":"#1e1e1e"}}>★ NEW ONLY</button>
<button onClick={()=>{setCatFilter(null);setNewOnly(false);}} style={{padding:"4px 12px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:8,fontFamily:"monospace",background:!catFilter&&!newOnly?"#e0e0e0":"transparent",color:!catFilter&&!newOnly?"#000":"#555",borderColor:!catFilter&&!newOnly?"#e0e0e0":"#1e1e1e"}}>ALL</button>
{Object.entries(CATS).map(([catId,cat])=>(
<button key={catId} onClick={()=>{setCatFilter(catFilter===catId?null:catId);setNewOnly(false);}} style={{padding:"4px 12px",borderRadius:3,border:"1px solid",cursor:"pointer",fontSize:8,fontFamily:"monospace",background:catFilter===catId?ACCENT+"22":"transparent",color:catFilter===catId?ACCENT:"#555",borderColor:catFilter===catId?ACCENT:"#1e1e1e"}}>{cat.label}</button>
))}
</div>
...
{[{label:"Total Wealth",value:`$${grandTotal.toLocaleString()}`,col:"#e0e0e0"},{label:"Accounts",value:"5 accounts",col:"#888"},{label:"Port. Beta",value:portBeta.toFixed(2),col:portBeta>1.2?"#f97316":portBeta>0.8?"#eab308":"#22c55e"},{label:"Macro Regime",value:`${CONV.label} ${TREND_SIG.arrow}`,col:CONV.color}].map(({label,value,col})=>(
<div key={label} style={{background:"#0a0a0a",borderRadius:5,padding:"10px 12px"}}>
<div style={{fontSize:7,color:"#444",fontFamily:"monospace",marginBottom:4}}>{label.toUpperCase()}</div>
<div style={{fontSize:12,fontWeight:800,color:col,fontFamily:"monospace"}}>{value}</div>
</div>
))}
</div>
<div style={{fontSize:7,color:"#444",fontFamily:"monospace",marginBottom:5}}>WEALTH BY ACCOUNT</div>
...
<div style={{fontSize:7,color:"#444",fontFamily:"monospace",marginBottom:5}}>ASSET CLASS MIX</div>
<div style={{display:"flex",height:10,borderRadius:3,overflow:"hidden",marginBottom:6}}>
{Object.entries(assetRollup).sort((a,b)=>b[1]-a[1]).map(([cls,val])=>(<div key={cls} style={{flex:val/grandTotal,background:rollupColors[cls]||"#888",opacity:0.85}}/>))}
</div>
<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
{Object.entries(assetRollup).sort((a,b)=>b[1]-a[1]).map(([cls,val])=>(
<div key={cls} style={{display:"flex",alignItems:"center",gap:3}}>
<div style={{width:6,height:6,borderRadius:"50%",background:rollupColors[cls]||"#888"}}/>
<span style={{fontSize:7,color:"#555",fontFamily:"monospace"}}>{cls} {(val/grandTotal*100).toFixed(0)}%</span>
</div>
))}
</div>
</div>
...
<span style={{fontSize:9,color:"#666",lineHeight:1.6}}>{text}</span>
</div>
))}
</div>
</div>
...
<div style={{fontSize:7,color:"#1e1e1e",fontFamily:"monospace"}}>Sample portfolio · Illustrative values only · Not investment advice</div>
</div>
)}
...
<div style={{fontSize:10,fontWeight:700,color:"#ccc",fontFamily:"monospace",marginBottom:8}}>{String(i+1).padStart(2,"0")} · {title}</div>
<div style={{fontSize:10,color:"#666",lineHeight:1.85}}>{body}</div>
</div>
))}
<div style={{fontSize:8,color:"#333",fontFamily:"monospace",textAlign:"center",padding:"8px 0"}}>
...
<span style={{fontSize:7,color:"#1e1e1e",fontFamily:"monospace"}}>SOURCES: FRED · CBOE · ICE BofA · FDIC · ISM · BLS · Shiller · Kim-Wright Fed · SLOOS Fed</span>
<span style={{fontSize:7,color:"#1e1e1e",fontFamily:"monospace"}}>⚠ NOT INVESTMENT ADVICE · v10</span>
</div>
</div>
);
}
