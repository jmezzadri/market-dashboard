/* Page · Macro Overview
   The five-domain backdrop. 27 indicators total. Filterable + drillable.  */

const PageMacro = ({ setPage }) => {
  const [view, setView] = useState("map");  // map | grid
  const [state, setState] = useState("all"); // all | extreme | elevated | calm
          ...
          {Object.entries(byDomain).map(([dom, inds]) => {
            ...
            return (
              <button key={dom}
                ...
                {elev > 0 && <div className="mc-domsub">+ <b>{elev}</b> elevated</div>}
      ...
      <section className="mt-pagesection" style={{ paddingTop: 16, paddingBottom: 8 }}>
        <div className="mc-filterbar">
          ...
          <div className="mc-legend">
            ...
            <div className="mt-pillgroup">
              {["All", "Rates", "Credit", "Equities", "Money", "Economy"].map(d => (
                <button key={d} className={`mt-pill ${domain === d ? "on" : ""}`} onClick={() => setDomain(d)}>{d}</button>
              ))}
            </div>
          </div>
          <div className="mc-legend" style={{ marginLeft: "auto" }}>
            ...
            <div className="mt-pillgroup">
              <button className={`mt-pill ${view === "map" ? "on" : ""}`} onClick={() => setView("map")}>Map</button>
              <button className={`mt-pill ${view === "grid" ? "on" : ""}`} onClick={() => setView("grid")}>Grid</button>
            </div>
          </div>
        </div>
      </section>
          ...
          <section className="mt-pagesection">
            <div className="lm-canvas">
              <RegimeCanvas data={positioned} onHover={setHoverInd} hover={hoverInd}
                            onSelect={setSelectedInd} selected={selectedInd} />
                ...
                <section key={dom} className="mt-pagesection">
                  <div className="mt-sectionhead">
                    ...
                    <div className="mc-domstate">
                      {visible.filter(i => i.state === "extreme").length > 0 && <span className="mt-tag mt-tag--extreme">{visible.filter(i => i.state === "extreme").length} extreme</span>}
                      {visible.filter(i => i.state === "elevated").length > 0 && <span className="mt-tag mt-tag--elev">{visible.filter(i => i.state === "elevated").length} elevated</span>}
                      {visible.filter(i => i.state === "calm").length > 0 && <span className="mt-tag mt-tag--calm">{visible.filter(i => i.state === "calm").length} calm</span>}
                    </div>
                  </div>
