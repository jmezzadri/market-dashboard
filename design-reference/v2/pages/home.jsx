/* Page · Home
   Today's tape at the top, then four feature cards (Macro, Tilt, Scanner,
   Portfolio), then market news. Each feature card opens its respective
   page. Hero is dense but breathable.                                    */
...
const PageHome = ({ setPage }) => {
  ...
  const move = useTickerJitter(78.4, 0.003, 1700);
  const cape = useTickerJitter(42.04, 0.0008, 2400);
    ...
    <div className="mt-pagebody">
      <section className="mt-pagehero">
        <div>
          ...
          <h1 className="mt-h1">
            Risk-on,<br /><i>inflationary</i> — with <span style={{ whiteSpace: "nowrap" }}>{stressed} of 27</span> flashing.
          </h1>
          <p className="mt-deck">
            Your portfolio beta is <Tip content="Beta vs S&P 500, weighted by position. Updated daily after close.">0.86</Tip>{" "}
            and the defensive sleeve is on <Tip content="Sleeve composition: 12% gold, 9% TLT, 4% cash. Would activate if MOVE crosses Watch (116).">standby</Tip>.
      ...
      {/* Today's read — indicators map (left) + nominal sector allocation (right) */}
      <section className="mt-pagesection">
        <div className="hm-todaygrid">
          <div className="lm-canvas hm-mapcard">
            ...
            <RegimeCanvas data={positioned} onHover={setHoverInd} hover={hoverInd}
              onSelect={setSelectedInd} selected={selectedInd} aspect={1.55} />
            <div className="lm-canvaslegend">
              ...
              <div className="lm-legrow lm-legrow--dim">{MT_INDICATORS.length} indicators · live · 5y normalized</div>
            </div>
          </div>
          ...
          <aside className="hm-tiltcard">
            <div className="hm-tiltcardhead">
              <div>
                ...
                <div className="hm-tiltcall">Risk On · <i>Inflationary</i></div>
            ...
            <div className="hm-allocgroup">
              ...
              {(() => {
                /* Engine's nominal allocation by sector. No SPY comparison. */
                const enriched = MT_SECTORS
                  .map(s => ({ ...s, alloc: Math.max(0, s.weight + s.tilt) }))
                ...
                return enriched.map(s => (
                  <button key={s.code} className="hm-allocrow" onClick={() => setPage("tilt")}>
                    ...
                    <span className="hm-allocbar">
                      <span style={{ width: `${(s.alloc / maxAlloc) * 100}%`,
                        background: "var(--mt-accent)" }} />
                    </span>
                    <span className="num hm-allocpct">{s.alloc.toFixed(1)}<i>%</i></span>
                  </button>
                ));
              })()}
            </div>
        ...
        {selectedInd && <IndicatorDetail ind={selectedInd} onClose={() => setSelectedInd(null)}
                                          onMethodology={() => setPage("methodology")} />}
      </section>

      {/* Feature blocks — the three remaining pillars (Macro + Tilt summarized above) */}
      <section className="mt-pagesection">
        <div className="hm-featgrid hm-featgrid--three">
          <FeatureCard
            num="01" label="Trading scanner" title="Five signals into one score"
            ...
            stat="13 long alerts"
            page="scanner" setPage={setPage} />
          
          <FeatureCard
            num="02" label="Portfolio insights" title="Your book, augmented"
            ...
            stat="$516K · +79.4% TTM"
            page="portfolio" setPage={setPage} />
          
          <FeatureCard
            num="03" label="Scenario analysis" title="Stress-test the playbook"
            ...
            page="scenarios" setPage={setPage} />
...
const FeatureCard = ({ num, label, title, body, stat, page, setPage, wide }) =>
<button className={`hm-feat ${wide ? "hm-feat--wide" : ""}`} onClick={() => setPage(page)}>
