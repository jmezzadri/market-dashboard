/* Page · All Indicators
   Full filterable, sortable table view of every indicator. The "registry"
   view — what / why / how each one is used.                              */

const PageIndicators = ({ setPage }) => {
  ...
  const filtered = useMemo(() => {
    return MT_INDICATORS.filter(i =>
      (cat === "All" || i.domain === cat) &&
      (layer === "All" || (layer === "Vol triggers" && ["MOVE · bond volatility","VIX","SKEW Index"].includes(i.name))
    ...
    <div className="mt-pagebody">
      <section className="mt-pagehero">
        ...
        <div className="al-summary">
          <FreshnessChip state="fresh" asOf="last poll &lt; 6 min" variant="pill" label={`${MT_INDICATORS.length} indicators`} />
          ...
          <div className="al-row">
            ...
            <div className="mt-pillgroup">
              {["All","Vol triggers","Cycle composite","Reference"].map(l => (
                <button key={l} className={`mt-pill ${layer === l ? "on" : ""}`} onClick={() => setLayer(l)}>{l}</button>
              ))}
            </div>
          </div>
          <div className="al-row">
            ...
            <div className="mt-pillgroup">
              {["All","Equities","Credit","Rates","Money","Economy"].map(c => (
                <button key={c} className={`mt-pill ${cat === c ? "on" : ""}`} onClick={() => setCat(c)}>{c}</button>
              ))}
            </div>
          </div>
          <div className="al-row" style={{ marginLeft: "auto" }}>
            <button className="mt-btn">＋ Filter</button>
            <button className="mt-btn">⚙ Columns <span className="sc-colcount num">11/14</span></button>
          </div>
        </div>
      </section>
          ...
          <table className="al-table">
            <thead>
              <tr>
                ...
                <th>Last refresh</th>
                <th className="num" onClick={() => setSort("current")}>Current {sort === "current" ? "↓" : ""}</th>
                <th className="num">3M ago</th><th className="num">6M ago</th><th className="num">12M ago</th>
                <th>5y</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <React.Fragment key={i.id}>
                    <tr className={`al-row-tr ${isOpen ? "open" : ""}`} onClick={() => setDrill(isOpen ? null : i.id)}>
                      ...
                      <td><span className={`al-type al-type--${i.state}`}>{i.state === "extreme" ? "LEAD" : i.state === "elevated" ? "COINC" : "LAG"}</span></td>
                      <td>
                        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <FreshnessChip state={i.fresh} asOf={i.asOf} />
                          <span className="num" style={{ color: "var(--mt-ink-1)", fontSize: 12 }}>{i.asOf}</span>
                        </span>
                      </td>
                      <td className={`num al-current al-current--${i.state}`}>
                        {i.value.toLocaleString(undefined, { maximumFractionDigits: 2 })}<span className="al-unit">{i.unit}</span>
                      </td>
                      <td className="num al-historical">{(i.value * 0.92).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      <td className="num al-historical">{(i.value * 0.88).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      <td className="num al-historical">{(i.value * 0.84).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                      <td>
                        <Sparkline data={i.trend} width={120} height={22}
                                   stroke={i.state === "extreme" ? "var(--mt-down)" : i.state === "elevated" ? "var(--mt-warn)" : "var(--mt-up)"} />
                      </td>
                    </tr>
                    {isOpen && (
                      <tr className="al-drill">
                        <td colSpan={10}><IndicatorDetail ind={i} onClose={() => setDrill(null)}
                                                          onMethodology={() => setPage("methodology")} /></td>
                      </tr>
                    )}
                  </React.Fragment>
                );
...
window.PageIndicators = PageIndicators;
