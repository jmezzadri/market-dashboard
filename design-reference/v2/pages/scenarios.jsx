/* Page · Scenario Analysis
   Pick a historical shock or build a custom one, see how the engine, S&P,
   and your portfolio would have responded.                                */

const PageScenarios = ({ setPage }) => {
      ...
      <section className="mt-pagehero">
        ...
        <div className="sn-picker">
          ...
          <div className="sn-scengrid">
            {MT_SCENARIOS.map(s => (
              <button key={s.id} className={`sn-scenpill ${active === s.id ? "on" : ""}`}
                      onClick={() => setActive(s.id)}>
                {s.name}
              </button>
            ))}
            <button className="sn-scenpill sn-scenpill--custom" onClick={() => setActive("custom")}>
          ...
          <div className="sn-customcard mt-card">
            {[
              ...
              <div key={k} className="sn-slidercell">
                ...
                <input type="range" min={mn} max={mx} step={step} value={custom[k]}
          ...
          <div className="sn-headercard mt-card">
            <div className="sn-headertop">
              <div>
                ...
                <div className="sn-headersub">
                  {scenarioBlurb(scen.id)}
                </div>
              </div>
              <div className="sn-headstats">
                ...
                <div><div className="mt-eyebrow">Horizon</div>
          ...
          <table className="sn-strategytable num">
            ...
            <tbody>
              ...
              <tr style={{ background: "var(--mt-accent-soft)" }}>
                <td style={{ textAlign: "left" }}><b>Your portfolio</b> <span className="mt-tag mt-tag--accent" style={{ marginLeft: 8 }}>YOU</span></td>
                <td>83.7%</td><td>15.8%</td><td>0.4%</td><td>—</td>
                <td className="down">−18.2%</td><td className="down">−28.4%</td>
              </tr>
              <tr><td style={{ textAlign: "left" }}><span style={{ color: "var(--mt-accent)" }}>Asset Tilt</span> <span className="mt-tag mt-tag--accent" style={{ marginLeft: 8 }}>MACROTILT</span></td>
                <td>40%</td><td>—</td><td>30%</td><td>30%</td>
                <td className="up">+4.6%</td><td className="down">−8.2%</td></tr>
            </tbody>
          </table>
        </div>
      </section>

      {/* Side-by-side: engine sector moves + portfolio P&L */}
      <section className="mt-pagesection">
        <div className="sn-splitcard">
          <article className="mt-card">
            ...
            <ul className="sn-engineimpact">
              {MT_SECTORS.slice(0, 8).map(s => {
                const proxy = Math.round((s.tilt * 4 + Math.random() * 6 - 3));
                const stress = -Math.abs(proxy) - Math.round(Math.random() * 10);
                return (
                  <li key={s.code}>
                    <span className="lm-flowcode">{s.code}</span>
                    <span className="sn-secname">{s.name}</span>
                    <span className={`num ${proxy >= 0 ? "up" : "down"} sn-proxy`}>{proxy > 0 ? "+" : ""}{proxy}%</span>
          ...
          <article className="mt-card">
            ...
            <table className="sn-poslosses num">
              <thead><tr><th style={{ textAlign: "left" }}>Ticker</th><th>Sector</th><th>Curr.</th><th>Stress</th><th>P/L</th></tr></thead>
              <tbody>
                {MT_POSITIONS.slice(0, 6).map(p => {
                  const loss = -Math.abs((p.value * 0.2) + Math.random() * (p.value * 0.1));
                  return (
                    <tr key={p.ticker}>
...
function scenarioBlurb(id) {
  return {
    ...
    dotcomflush: "October 2002: capitulation low after 32-month grind. Tech multiples re-rated by 60%+.",
...
function scenarioDD(id) {
  return { blackmonday: "−22.6%", dotcomup: "+8.4%", dotcomflush: "−14.1%", gfc: "−37.4%", ratehike: "−19.8%", covid: "−33.9%", inflation: "−24.1%", ai: "−18.2%" }[id];
}
function scenarioCall(id) {
  return { blackmonday: "Risk Off · Deflationary", dotcomup: "Risk On · Inflationary", dotcomflush: "Risk Off · Neutral", gfc: "Risk Off · Deflationary", ratehike: "Watch · Neutral", covid: "Risk Off · Deflationary", inflation: "Watch · Inflationary", ai: "Watch · Neutral" }[id];
}
